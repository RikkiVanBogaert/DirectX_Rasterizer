...
namespace dae {
		...
		m_pWindow(pWindow)
	{
		...
		m_Camera.Initialize(45.f,  {0, 0, -50.f}, float(m_Width) / m_Height );

		Effect* pShadedEffect{ new Effect(m_pDevice, L"Resources/PosCol3D.fx") };
	...
	void Renderer::Update(const Timer* pTimer)
	{
		...
		for (auto& m : m_pMeshes)
		{
			m->Update(m_Camera.GetViewProjectionMatrix(), m_Camera.GetViewInvertMatrix());
		}
	}
